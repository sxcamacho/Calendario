...
(function ($) {
    ...
    $.widget("ui.weekCalendar", {
        ...
        ***********************/
        ...
        verModoDia: function () {
            ...
            var fecha = this.element.data("startDate");
            this.options.firstDayOfWeek = fecha.getDay();
            this.options.daysToShow = 1;
            this.options.modeDay = true;
            $(this.element).empty();
            this._init();
            //Agregado 08-07-2014 scamacho
            this._clearCalendar();
            //Fin del agregado
            this._loadCalEvents(fecha);

        },

        verModoSemana: function () {

            $.cookie('verModo', 'semana');

            var fecha = this.element.data("startDate");
            this.options.firstDayOfWeek = 1;
            this.options.daysToShow = 5;
            this.options.modeDay = false;
        ...
        */
        ...
        /* Hecho por scamacho
        * Go to the previous day relative to the currently displayed week
        */
        prevDay: function () {
            //minus more than 1 day to be sure we're in previous week - account for daylight savings or other anomolies
            var days = 1;

            if (this.element.data("startDate").getDay() == 1) {
                days = 3;
            }

            var milisegundos = parseInt(days * 24 * 60 * 60 * 1000);
            var newDate = new Date(this.element.data("startDate").getTime() - (milisegundos));

            //agregado scamacho
            this.options.firstDayOfWeek = newDate.getDay();
        ...
        */
        nextDay: function () {
            ...
            if (this.element.data("startDate").getDay() == 5) {
                days = 3;
            }
        ...
        */
        _renderCalendar: function () {
            ...
            if (options.buttons) {
                calendarNavHtml = "<nav class=\"navbar navbar-default navbar-top\" role=\"navigation\" style=\"margin-bottom: 0px; border-bottom-width: 0px; margin-top: 20px;\">\
							<div class=\"wc-nav navbar-inner\" >\
							<ul class=\"nav\">\
							<button class=\"wc-modeday btn\">" + options.buttonText.modeday + "</button>\
							<button class=\"wc-modeweek btn\">" + options.buttonText.modeweek + "</button>\
							<button class=\"wc-today btn\">" + options.buttonText.today + "</button>\
							<button class=\"wc-prev btn\">" + options.buttonText.lastWeek + "</button>\
							<button class=\"wc-next btn\">" + options.buttonText.nextWeek + "</button>\
							</div>\
							</nav>";
                ...
                $calendarContainer.find(".wc-nav .wc-modeday").click(function () {
                    self.element.weekCalendar("verModoDia");
                    return false;
                });

                $calendarContainer.find(".wc-nav .wc-modeweek").click(function () {
                    self.element.weekCalendar("verModoSemana");
                    return false;
                });

                $calendarContainer.find(".wc-nav .wc-prev").click(function () {
                    if (options.modeDay == true) {
        ...
        */
        _refreshEventDetails: function (calEvent, $calEvent) {
            ...
            /* el 09/11/2013 agregamos este codigo para en el caso de sesiones de menos de 15 minutos inclusive
            se muestre la hora inicial y el titulo de la sesion.
            */
            var self = this;
            var options = this.options;
    ...
    $.extend($.ui.weekCalendar, {
        ...
        defaults: {
            ...
            longDays: ['Domingo', 'Lunes', 'Martes', 'Miercoles', 'Jueves', 'Viernes', 'Sábado']
        }
    });
